...
import cv2
from utils.image_processing import cvtColor, preprocess_input
...
def load_dataset(dataset_path):
    ...
    for img_file in os.listdir(img_path):
        ...
        if not os.path.exists(full_label_path):
            continue  # 如果标签文件不存在，跳过当前循环
        # 读取图像
        img = Image.open(os.path.join(img_path, img_file))
        # 读取标签
        with open(label_file, 'r') as f:
            labels = np.array([line.split() for line in f.read().splitlines()], dtype=np.float32)
...
class YoloDataset(Dataset):
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        #   获得预测框
        # ------------------------------#
        box = np.array([np.array(list(map(float, box.split(',')))) for box in parts[1:]])
